...
class BeuhlerClock:
    ...
    def _feedback_theta_deg(self, motor: int) -> float:
        """Return the latest measured leg angle (deg) if available."""
        ...
        if state is None:
            return 0.0
        return _wrap_to_180(math.degrees(float(state.position)))
    ...
    def _run(self) -> None:
        ...
        self._theta_a_deg = self._feedback_theta_deg(2)
        self._theta_b_deg = _wrap_to_180(self._theta_a_deg + self.group_offset_deg)
        ...
        while not self._stop_event.is_set():
            ...
            self._theta_a_deg = _wrap_to_180(theta_a + vA * control_dt)
            self._theta_b_deg = _wrap_to_180(theta_b + vB * control_dt)
