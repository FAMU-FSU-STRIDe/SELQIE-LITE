...
class GliderController:
    ...
    def _wait_for_depth_target_from_deeper(self, target_depth_m: float) -> None:
        reached_deeper = False
        while rclpy.ok():
            current_depth = self._depth()
            if current_depth is None:
                time.sleep(0.1)
                continue
            if not reached_deeper:
                if current_depth > target_depth_m:
                    reached_deeper = True
                time.sleep(0.1)
                continue
            if current_depth <= target_depth_m:
    ...
    def run(self) -> None:
        ...
        self._wait_for_depth_target_from_deeper(0.4)
        self._set_lights(COLOR_BLUE)
        time.sleep(5.0)
